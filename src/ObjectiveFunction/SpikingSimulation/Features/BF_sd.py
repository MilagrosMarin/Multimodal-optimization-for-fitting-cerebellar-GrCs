...
class BF_sd (Feature.Feature):

    def __init__(self,name, simulation, config_dict):
        super(BF_sd, self).__init__(name,simulation, config_dict)
        return
    ...
    def calculate(self):
        ...
        sdeviation_per_neuron = numpy.zeros((num_neurons))
        ...
        for neuid in range(num_neurons):
            ...
            sdeviation_per_neuron[neuid] = numpy.std(freq)

        layer_average = numpy.average(burst_freq_per_neuron)

        layer_average_sd = numpy.average(sdeviation_per_neuron)

        logger.debug('Obtained average burst frequencies in layer %s: Average per neuron: %s Mean burst frequency: %s +- %s', self.layer, str(burst_freq_per_neuron), str(layer_average), str(layer_average_sd))

        score = (abs(layer_average-self.target_value)*self.weight)*(layer_average_sd+1)

        logger.debug('Calculated score %s in layer %s', str(score),self.layer)

        #logger.error('%s= %s #(# cycles %s with Mean Freq = %s +- %s; score_BF=%s', self.name, freq, (end_cycle-init_cycle),str(layer_average), str(layer_average_sd),str(abs(layer_average-self.target_value)*self.weight))
    ...
    def calculate_feature(self):
        ...
        return [layer_average, layer_average_sd]
